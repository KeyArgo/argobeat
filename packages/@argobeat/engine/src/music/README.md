...
AI-powered music generation module for ArgoBeat. Generates unique, professional-quality music for each session using pluggable backends (MusicGen, Stable Audio, etc.).
...
│                    PromptProvider                           │
│  ┌──────────────────┐  ┌────────────────────────────────┐  │
│  │ StaticPromptProv  │  │ ClaudePromptProvider (optional) │  │
│  │ (zero deps)       │  │ (requires @anthropic-ai/sdk)    │  │
│  └────────┬─────────┘  └──────────────┬─────────────────┘  │
│           └──────────┬─────────────────┘                    │
│                      ▼                                      │
│              MusicGenerator                                 │
...
```typescript
import { ArgoBeatEngine, ClaudePromptProvider } from '@argobeat/engine';
import Anthropic from '@anthropic-ai/sdk';

const claude = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });
const provider = new ClaudePromptProvider(claude);
