...
async function render(): Promise<Float32Array[]> {
  ...
  // AM entrainment bus — sustained layers (pads + bass) only. NOT master bus (would pump transients).
  // Brain.fm applies amplitude modulation at target Hz directly to audio; measured power share:
  //   focus 1.36%, deepWork 0.52%, relax 0.43%, meditate 1.16%, sleep ~1.2% (2026-05-31).
  const AM_HZ:    Record<string, number> = { focus: 15, deepWork: 18, relax: 10, meditate: 6, sleep: 2 };
  // Depths tuned to match measured brain.fm power share per mood (focus 1.36%, deepWork 0.52%,
  // relax 0.43%, meditate 1.16%, sleep ~1.2%). Small values keep it subliminal, not a tremolo.
  const AM_DEPTH: Record<string, number> = { focus: 0.05, deepWork: 0.03, relax: 0.03, meditate: 0.05, sleep: 0.015 };
  const amHz    = AM_HZ[mood]    ?? 15;
  const amDepth = AM_DEPTH[mood] ?? 0.25;

  const amBus = ctx.createGain();
  amBus.gain.value = 1.0;
  amBus.connect(masterInput);

  // LFO drives amBus.gain: gain = 1 + depth*sin(2π*amHz*t). Use two slightly-detuned sines
  // to avoid mechanical tremolo sound while keeping a clean spectral peak at amHz.
  const amLfo1 = ctx.createOscillator();
  amLfo1.type = 'sine';
  amLfo1.frequency.value = amHz;
  const amLfo2 = ctx.createOscillator();
  amLfo2.type = 'sine';
  amLfo2.frequency.value = amHz * 1.007; // slight detune = warmer, less mechanical

  const amDepthGain1 = ctx.createGain();
  amDepthGain1.gain.value = amDepth * 0.7;
  const amDepthGain2 = ctx.createGain();
  amDepthGain2.gain.value = amDepth * 0.3;

  amLfo1.connect(amDepthGain1);
  amLfo2.connect(amDepthGain2);
  amDepthGain1.connect(amBus.gain);
  amDepthGain2.connect(amBus.gain);
  amLfo1.start(0);
  amLfo2.start(0);
  amLfo1.stop(durationS);
  amLfo2.stop(durationS);
  ...
  for (let beat = 0; beat < totalBeats; beat++) {
    ...
    if (beat % 16 === 0) {
      ...
      // Play pad chord — routed through amBus so AM modulates sustained layers only
      for (const interval of voicing) {
          const noteFreq = rootFreq * Math.pow(2, interval / 12);
        if (noteFreq > 20 && noteFreq < 4000) {
          playOfflinePad(ctx, amBus, noteFreq, time, beatDuration * 15.5, 0.03, config.padFilterHz * 1.22 * padFilterMult, {
    ...
    // Bass pulse on beats 1 and 3 — routed through amBus (sustained harmonic layer)
    if (beatInBar === 0 || (beatInBar === 2 && rng.next() < 0.45)) {
      const bassFreq = (scaleNotes[0]?.freq ?? freqs[0]) / 2;
        playOfflineBass(ctx, amBus, bassFreq, time, beatDuration * 0.72, beatInBar === 0 ? 0.044 : 0.026);
    }
  ...
  // Explicit lowpass drift removed per huddle 2026-05-31: user wants stable, non-changing texture.
  // Natural centroid variation comes from generative Markov melody/chord changes, not a filter sweep.
  // The reference track's +23% centroid drift is an emergent artifact of note events, not a deliberate
  // architectural sweep — we should not replicate it with an explicit LFO/ramp.

  // Fade in (first 2 seconds) and fade out (last 2 seconds)
  masterGain.gain.setValueAtTime(0.001, 0);
  masterGain.gain.linearRampToValueAtTime(masterGainValue, 2);
  masterGain.gain.setValueAtTime(masterGainValue, durationS - 2);
...
async function main() {
  try {
    ...
    // Post-render LUFS normalize to match measured brain.fm level (~-29 LUFS / -26 dBFS RMS).
    // Applied AFTER rendering, NOT via an in-graph limiter (would fight the tanh saturator).
    // Brain.fm reference tracks measured at ~-29 LUFS; old output was ~-37 (too quiet).
    const TARGET_RMS_DBFS = -26; // → ~-29 LUFS for this material
    const CEIL_PEAK = 0.891;     // -1 dBFS true-peak ceiling
    let sumSq = 0, nSamples = 0;
    for (const ch of channels) { for (const s of ch) { sumSq += s * s; nSamples++; } }
    const curRmsDb = 20 * Math.log10(Math.sqrt(sumSq / nSamples) + 1e-10);
    const makeupGain = Math.pow(10, (TARGET_RMS_DBFS - curRmsDb) / 20);
    for (const ch of channels) { for (let i = 0; i < ch.length; i++) ch[i] *= makeupGain; }
    let peak = 0;
    for (const ch of channels) { for (const s of ch) { const a = Math.abs(s); if (a > peak) peak = a; } }
    if (peak > CEIL_PEAK) {
      const scale = CEIL_PEAK / peak;
      for (const ch of channels) { for (let i = 0; i < ch.length; i++) ch[i] *= scale; }
    }
